...
export const Login: React.FC = () => {
  ...
  const handleBiometricLogin = async () => {
    ...
    try {
      ...
      if (!ok) {
        setBiometricError('Verificación biométrica fallida. Intentá de nuevo.');
        return;
      }
      // Biometric passed — re-sign in with saved email via Firebase
      ...
      if (auth && auth.currentUser) {
        // Session still valid — biometric was just a gate, we're in!
        return; // Auth state listener in App.tsx will handle navigation
      } else {
        // Session expired — can't auto-login without password, fall back
        setBiometricError('Tu sesión expiró. Por favor ingresá con usuario y contraseña.');
        clearBiometricCredential();
        setBiometricScreen(false);
      }
    } catch {
      setBiometricError('Error al verificar biometría. Intentá con contraseña.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRegisterBiometric = async () => {
    ...
    if (!ok) {
      // User cancelled or not supported — just proceed normally
    }
  };
      ...
      {showBiometricPrompt && (
        <div className="fixed inset-0 z-[500] flex items-end sm:items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] shadow-2xl p-8 animate-slide-up text-center">
            ...
            <h3 className="text-xl font-black text-slate-900 mb-2">Acceso Biométrico</h3>
            <p className="text-sm text-slate-500 font-medium mb-6 leading-relaxed">
              ¿Querés usar <strong>Face ID o huella dactilar</strong> para entrar más rápido la próxima vez?
            </p>
        ...
        <div className="min-h-screen w-full flex items-center justify-center p-6 bg-gradient-to-br from-slate-50 to-primary-50 relative overflow-hidden">
          ...
          <div className="w-full max-w-sm text-center animate-slide-up">
            ...
            <div className="bg-white rounded-[2rem] p-8 shadow-xl shadow-slate-200/60 mb-4">
              ...
              <h2 className="text-lg font-black text-slate-900 mb-2">Verificación Biométrica</h2>
              <p className="text-sm text-slate-400 font-medium mb-6">Usá Face ID o tu huella dactilar para acceder</p>
              ...
              >
                <Fingerprint size={22} /> {loading ? 'Verificando...' : 'Acceder con Biometría'}
              </button>
            </div>
            ...
            >
              <X size={12} /> Usar usuario y contraseña
            </button>
          </div>
        </div>
      )}
            ...
            <div className="glass-card p-10 lg:p-12">
              ...
              {mode === 'PATIENT' ? (
                <form onSubmit={handlePatientLogin} className="space-y-6">
                  <div>
                    <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">Número de DNI</label>
                  ...
                  <div>
                    <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">Contraseña</label>
                    <div className="relative group">
                ...
                <form onSubmit={handleStaffLogin} className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="relative group">
                      <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5 group-focus-within:text-primary-500 transition-colors" />
                      <input type="password" value={staffPass} onChange={(e) => setStaffPass(e.target.value)} placeholder="••••••••" className="w-full bg-slate-50/50 border border-slate-200 rounded-2xl py-4 pl-12 pr-4 text-slate-900 font-bold placeholder:text-slate-400 focus:outline-none focus:ring-4 focus:ring-primary-500/20 focus:border-primary-500 transition-all font-sans tracking-widest" />
                    </div>
                  </div>
                  {error && <div className="bg-red-50 border border-red-200 text-red-600 text-sm font-semibold p-4 rounded-2xl text-center animate-fade-in flex items-center justify-center gap-2"><ShieldCheck className="w-5 h-5" />{error}</div>}
